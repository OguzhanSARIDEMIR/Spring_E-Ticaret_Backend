...
public class BasketService {
    ...
    public ResponseEntity add(Basket basket) {
        ...
        if (optionalProduct.isPresent()) {
            for (Basket basketItem : baskets) {
                if (basketItem.getProduct().getPid() == basket.getProduct().getPid()) {
                    isSameProduct = true;
            ...
            if (basketQuantity <= stockQuantity) {
                ...
                hm.put(REnum.message, "Yeterli stok yok");
                return new ResponseEntity<>(hm, HttpStatus.OK);
            }
            ...
            hm.put(REnum.status, false);
            hm.put(REnum.message, "öyle bir ürün yok");
            return new ResponseEntity<>(hm, HttpStatus.OK);
        }
    ...
    public ResponseEntity delete(Long id) {
        ...
        if (optionalBasket.isPresent()) {
            ...
            hm.put(REnum.status, false);
            hm.put(REnum.message, "Böyle bir sepet yok");
    ...
    public ResponseEntity update(Long id, Integer quantity) {
        ...
        try {
            ...
            if (optionalBasket.isPresent()) {
                ...
                hm.put(REnum.status, true);
                hm.put(REnum.message, "Güncelleme başarılı");
    ...
    public ResponseEntity getBasketList_by_User(String email) {
    ...
    public ResponseEntity bulkDeleteBasket() {
        ...
        hm.put(REnum.status, true);
        hm.put(REnum.message, "Sepet Silindi");
       return new ResponseEntity<>(hm, HttpStatus.OK);
